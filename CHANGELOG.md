...
- **Le service d'inférence n'échoue plus OUVERT** (passe sécurité S1.1,
  `docs/archive/PASSE_SECURITE_2026-08.md`). Deux défauts fail-open sont inversés :
...
(`docs/archive/REFACTORING_QUALITE.md`) est livré **en intégralité — 15 vagues** : le code est
...
- **Interface multilingue (i18n) — socle (français + anglais)** : l'interface se traduit via
  **Flask-Babel** (gettext). Un **sélecteur de langue** apparaît dans la barre de navigation ;
  ...
  Voir `docs/archive/I18N_MULTILANGUE.md`. **Couverture** : toute l'interface (navigation, connexion,
...
- **Sauvegardes dans l'interface (admin)** : nouvelle page **Administration → Maintenance**
  ...
  `maintenance.backup_dir` (défaut `./backups`).
- **Sauvegarde planifiée (timer systemd)** : nouvelle section de config `maintenance.schedule`
  (`enabled`, `on_calendar`, `keep`, `exclude_audio`) et commande
...
- **Éditeur de transcription intégré** (`/jobs/<id>/editor`, cadrage
  `docs/archive/EDITEUR_SRT_INTEGRE.md`) : correction du texte, des locuteurs, du découpage et
...
- **Profils de traitement** (remplacent le binaire `fast`/`quality`) — jalon vers la 1.0, sur
  ...
  retombe sur le comportement complet. Cadrage : `docs/archive/PROFILS_TRAITEMENT_WORKFLOW.md`. Restent la
...
**Tests de charge (concurrence) des topologies — `docs/archive/PLAN_TEST_CHARGE.md`.** Campagne de stress
...
- **Banc de test « topologie split GPU » entièrement containerisée** (`docker-compose.split-gpu.yml`
  ...
  **STT Cohere via vLLM**) + **LLM d'arbitrage Qwen3.6-27B-FP8 via vLLM** (TP=4, FP8 Marlin), PostgreSQL
  ...
  plan de contrôle + job son réel), `docs/archive/PLAN_TEST_SPLIT_VLLM.md`.
