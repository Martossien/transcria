...
⚠️ **Beta — dernière release : [`v0.1.0-beta.7`](https://github.com/Martossien/transcria/releases/tag/v0.1.0-beta.7).** Nouveau en beta.7 : la LLM d'arbitrage est désormais **multi-backend — Ollama / llama.cpp / vLLM — choisie automatiquement selon le matériel physique** via un **catalogue de profils en données** unique (`transcria/data/llm_profiles.yaml`, **aucune taille de modèle en dur** ; empreinte VRAM dérivée du modèle réel et re-mesurée au 1ᵉʳ chargement). **Ollama devient le backend « facile »** (`curl | sh`, sans compilation, sans `nvcc`, sans token HF) ; llama.cpp gagne une **voie binaire CUDA précompilé** (ai-dock, sha256 vérifié — s'installe sur distro vierge sans compiler) ; voir [docs/LLM_BACKENDS.md](docs/LLM_BACKENDS.md). Le produit est fonctionnel et couvert par **2 983 tests (CI verte : ruff, mypy, pytest complet sur PostgreSQL, ~80 % de couverture)**. L'installeur est validé de bout en bout sur **Ubuntu 22.04/24.04, Debian 12, Fedora 41, Rocky 9 × Python 3.11–3.13** (apt + dnf), pipeline complet STT + diarisation + LLM, en mono- et multi-GPU (Ollama 12B/35B, llama.cpp 35B-A3B, vLLM 27B-FP8) — voir [docs/LLM_PROFILS_VALIDATION.md](docs/LLM_PROFILS_VALIDATION.md). La **topologie distribuée** (frontale CPU + nœud de ressources GPU) est validée **de bout en bout sur audio réel** avec une LLM d'arbitrage vLLM (tensor-parallel) et placement VRAM automatique sur 8 GPU — voir [docs/DOCKER.md](docs/DOCKER.md) et [docs/PLAN_TEST_SPLIT_VLLM.md](docs/PLAN_TEST_SPLIT_VLLM.md). Selon le SemVer, la **série `0.x` est une phase de stabilisation** : l'API, le schéma de configuration et le modèle de données peuvent encore évoluer sans garantie de compatibilité ascendante jusqu'à `1.0.0`. À évaluer, à piloter — pas à mettre en production sans votre propre validation.
...
- **Tests et benchmarks** : suite pytest mockée (2 692 tests, CI verte), E2E GPU réel, E2E automatisés sans GPU pour le pipeline DOCX, runner benchmark multi-combinaisons pour comparer Cohere/Whisper/Granite et les options audio.
...
Une fois le traitement terminé, la page **Résultats & affinage** (accessible depuis l'étape Export et depuis la liste des traitements) présente les livrables et le **chat d'affinage** : discuter du contenu avec la LLM locale, appliquer une correction sur tous les livrables à la fois, ajuster les options de rendu du rapport et restaurer une version antérieure si besoin.
