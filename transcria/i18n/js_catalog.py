"""Catalogue des chaînes traduites exposées au JavaScript (déplacé de transcria/web/i18n_js.py — vague A1).

Le front appelle ``t("chaîne source française")`` (helper static/js/i18n.js). Ici on liste
les **chaînes source** (msgid = français, convention gettext) utilisées côté navigateur, et on
construit ``window.I18N = { source: traduction }`` pour la locale courante.

Source UNIQUE de vérité = les mêmes catalogues gettext que le reste de l'UI. Les chaînes sont
marquées avec ``N_`` (gettext-noop, mot-clé extrait par pybabel) pour apparaître dans le
catalogue **sans** être traduites à l'import — la traduction se fait par requête dans
``build_js_catalog``. Quand on ajoute un ``t("…")`` dans un .js, on ajoute son source ici.
"""
from __future__ import annotations

import json

from flask_babel import gettext


def N_(s: str) -> str:
    """Marqueur d'extraction gettext (no-op) : pybabel récolte l'argument, renvoie tel quel."""
    return s


# Chaînes utilisées dans wizard.js / wizard-api.js (français source). Tenu à jour à la main,
# vague après vague. Beaucoup recoupent des msgid déjà présents côté templates (dédupliqués
# dans le catalogue) ; les nouvelles sont propres au JS.
JS_MESSAGES: tuple[str, ...] = (
    N_('%(label)s (%(elapsed)s écoulées)'),
    N_('%(n)s diapo(s)'),
    N_('%(n)s image(s) ignorée(s)'),
    N_('%(n)s page(s)'),
    N_('(créé)'),
    N_('Ajouter cette forme validée à un lexique central, partagé et réutilisé sur les prochains jobs'),
    N_('Annuler'),
    N_('Annuler le traitement'),
    N_('Au lexique central'),
    N_('Catégorie'),
    N_('Catégorie libre'),
    N_('Ce job a déjà été traité.'),
    N_('Contexte projet'),
    N_('Contexte proposé (%(n)s)'),
    N_('Continuer à attendre'),
    N_('Correction LLM en cours…'),
    N_('Démarrage…'),
    N_('Détection des locuteurs lancée sur le nœud GPU — la page se rafraîchira.'),
    N_('Erreur :'),
    N_('Erreur : %(e)s'),
    N_('Erreur réseau : %(msg)s'),
    N_('Erreur réseau.'),
    N_('Erreur serveur (%(status)s).'),
    N_('Erreur serveur non JSON.'),
    N_('Ex: Forme douteuse A, forme douteuse B'),
    N_('Ex: Forme validée'),
    N_('Export en cours…'),
    N_('Extrait %(n)s :'),
    N_('Extrait indisponible'),
    N_('Fichier trop volumineux (dépasse la limite du serveur).'),
    N_('Fichier téléversé. Rechargement…'),
    N_('Finalisation…'),
    N_('Fonction'),
    N_('Forme validée'),
    N_('Formes suspectes observées'),
    N_('Identification des locuteurs…'),
    N_('Informations client'),
    N_('Informations entretien'),
    N_('Informations formation'),
    N_('Informations incident'),
    N_('Informations légales PV'),
    N_('Informations légales PV — Séance extraordinaire'),
    N_('Informations négociation'),
    N_('Informations séminaire'),
    N_('La LLM est peut-être en boucle. Si les fichiers de correction sont déjà produits, vous pouvez annuler — le job sera récupéré automatiquement au redémarrage du service.'),  # noqa: E501
    N_("Le résumé n'a pas pu être généré (LLM sans production après 3 tentatives). La transcription est conservée — vous pouvez relancer."),
    N_('Lexique central : %(name)s'),
    N_('Nom'),
    N_('Ordre du jour & indicateurs'),
    N_('Oui, relancer'),
    N_('Pourquoi cette forme doit être validée'),
    N_('Priorité'),
    N_('Produit'),
    N_("Proposé à partir d'un document que vous avez joint à la réunion"),
    N_('Rapport qualité…'),
    N_('Relancement du traitement…'),
    N_('Relancer le résumé'),
    N_("Renseignez d'abord la forme validée."),
    N_('Retirer'),
    N_('Réponse serveur inattendue (HTTP %(status)s).'),
    N_('Réponse serveur invalide.'),
    N_('Rôle dans la réunion'),
    N_('Session expirée — redirection vers la connexion…'),
    N_('Terminé.'),
    N_('Traitement annulé après %(elapsed)s.'),
    N_('Traitement annulé.'),
    N_('Traitement en cours'),
    N_('Traitement démarré. Transcription ASR en cours… (0s écoulées)'),
    N_('Traitement relancé. Transcription ASR en cours… (0s écoulées)'),
    N_('Traitement terminé en %(elapsed)s. Chargement…'),
    N_('Transcription ASR en cours…'),
    N_('Résumé de contrôle en cours de génération (lancé automatiquement)…'),
    N_("VRAM insuffisante : l'administrateur a été prévenu. Le résumé reprendra automatiquement dès que la mémoire GPU sera libérée."),
    N_('Voulez-vous relancer le traitement ? (le lexique et les corrections actuels seront appliqués)'),
    N_('issu des documents fournis'),
    N_('mis à jour il y a %(n)sh'),
    N_('mis à jour il y a %(n)smin'),
    N_('mis à jour il y a %(n)ss'),
    N_('Position %(n)s dans la file'),
    N_('démarrage estimé dans ~%(d)s'),
    N_('démarrage imminent'),
    N_('moins d’une minute'),
    N_('%(n)s min'),
    N_('%(h)s h %(m)s min'),
    N_('%(h)s h'),
    N_("⚠ L'attente GPU dépasse la limite configurée — vous pouvez patienter ou soumettre en profil express (moins de ressources)"),
    N_('nouveau'),
    N_('requête impossible'),
    N_('sans timecode'),
    N_('tronqué'),
    N_('· %(listened)s/%(total)s écoutés'),
    N_("Échec de l'ajout du document."),
    N_('Échec du traitement après %(elapsed)s.'),
    N_('Échec du traitement.'),
    N_('Écouter 5 secondes avant et après'),
    N_('Écouté'),
    N_('⚠ Le traitement LLM prend plus de temps que prévu (%(elapsed)s).'),

    # --- Vague 2 : srt_editor.js + meeting_types.js ---
    N_('%(n)s type(s) visible(s) — quota : %(max)s créés/personne'),
    N_('%(seg)s segments · %(spk)s locuteurs'),
    N_('Ajuster début/fin'),
    N_('Aller au repère'),
    N_('Aller à cet instant'),
    N_('Aperçu impossible.'),
    N_('Appliquer cette palette'),
    N_("Audio non disponible sur cette installation — l'écoute et la forme d'onde sont désactivées, toutes les éditions restent possibles."),  # noqa: E501
    N_('Avancer ce segment ET tous les suivants de 500 ms'),
    N_('Avertissements (non bloquants) : %(w)s'),
    N_('Bande du bas : molette = zoom · glisser les bords bleus = retimer le segment actif'),
    N_('COMPTE-RENDU DE TRANSCRIPTION'),
    N_('Ce dossier est édité dans un autre onglet ou par une autre personne — brouillon suspendu ici pour ne rien écraser.'),
    N_('Changer le locuteur'),
    N_('Commence avant la fin du segment précédent'),
    N_('Contenu extrait (ordre du jour, décisions, votes…)'),
    N_('Contexte de la réunion'),
    N_('Couper au curseur (C)'),
    N_('Créer le mien'),
    N_('Créer mon type (à partir de « %(name)s »)'),
    N_('Ctrl+S — enregistrer une version'),
    N_('Ctrl+Z / Ctrl+Y — annuler / rétablir'),
    N_('Descendre'),
    N_("Donnez d'abord un nom au type."),
    N_('Début'),
    N_('Enregistrement impossible.'),
    N_("Entrée — valider le texte et reprendre l'écoute"),
    N_('Espace — lecture/pause'),
    N_('Exporter (partage entre installations / communauté)'),
    N_('Fin'),
    N_('Fusion possible sur une plage contiguë seulement'),
    N_('Fusionner avec le précédent'),
    N_('Fusionner la sélection'),
    N_('Groupe %(g)s'),
    N_('Import impossible.'),
    N_('Importé — à relire'),
    N_('Impossible de charger la transcription.'),
    N_('Informations spécifiques (champs du type)'),
    N_('Intégré'),
    N_("La transcription a changé depuis (correction ou affinage) : reprendre le brouillon écrasera ces changements à l'enregistrement."),
    N_('Libellé (ex. Budgets évoqués)'),
    N_('Libellé (ex. Filiale concernée)'),
    N_('Libellé court (Word)'),
    N_('Logo refusé.'),
    N_('M — poser un repère · G — aller au temps'),
    N_('Marquer comme traité'),
    N_('Micro inaccessible : autorisez le microphone puis réessayez.'),
    N_('Modifier « %(name)s »'),
    N_('Modifié depuis la dernière version'),
    N_('Monter'),
    N_('Nombre'),
    N_('Nouveau locuteur…'),
    N_('Partage impossible.'),
    N_('Partager au groupe %(g)s'),
    N_('Partager à tous'),
    N_('Partagé à tous'),
    N_('Participants & locuteurs'),
    N_('Points à vérifier'),
    N_('Privé'),
    N_('Raccourcis :'),
    N_('Reculer ce segment ET tous les suivants de 500 ms'),
    N_('Rendre privé'),
    N_('Repère %(n)s — %(t)s'),
    N_('Retirer'),
    N_('S / E — caler le début / la fin sur la lecture'),
    N_('S/E : caler début/fin sur la lecture'),
    N_('Supprimer %(n)s segments ?'),
    N_('Supprimer ce segment'),
    N_('Supprimer le segment #%(n)s et son texte ?'),
    N_('Supprimer le type « %(name)s » ? Les traitements passés le conservent.'),
    N_('Synthèse (en section autonome)'),
    N_('Tab / Maj+Tab — segment suivant / précédent'),
    N_('Texte'),
    N_('Texte long'),
    N_('Toujours présent'),
    N_("TranscrIA · Réunion d'exemple · Page 1/4"),
    N_('Transcription'),
    N_("Type « %(name)s » importé (privé, à relire) : ouvrez-le, vérifiez, enregistrez pour l'activer."),
    N_('Télécharger un exemple Word'),
    N_('Un brouillon du <strong>%(when)s</strong> (%(n)s segments) a été trouvé.'),
    N_("Un traitement est en cours sur ce dossier — l'éditeur est en lecture seule et rouvrira en écriture à la fin."),
    N_('Voix retenue'),
    # --- Couche fiabilité de l'éditeur (2026-08) ---
    N_('segment très court'),
    N_('segment court et pauvre en mots'),
    N_('débit de parole anormal'),
    N_('probabilité de non-parole élevée'),
    N_('mots à faible confiance'),
    N_('texte non latin'),
    N_("motif d'hallucination générique"),
    N_("signature d'hallucination connue de ce moteur"),
    N_('zone audio dégradée (diagnostic)'),
    N_('marqué [INCERTAIN] par la correction'),
    N_('douteux'),
    N_('à vérifier'),
    N_('%(n)s à vérifier'),
    N_('restauré'),
    N_('restaurer'),
    N_('%(n)s segment(s) retirés comme hallucinations confirmées — vérifier / restaurer'),
    N_('Retirés sur double preuve (signature du moteur + zone sans parole). '
       'La restauration réinsère le texte à sa place — enregistrez ensuite une version.'),
    N_('aucune modification'),
    N_('brouillon enregistré à %(t)s'),
    N_('brouillon non enregistré — nouvelle tentative'),
    N_('brouillon repris — pensez à enregistrer une version'),
    N_('brouillon suspendu (édition ailleurs)'),
    N_('chevauche'),
    N_('enregistrement de la version…'),
    N_('enregistrement impossible'),
    N_('ex. %(name)s — mon équipe'),
    N_('modifications non enregistrées'),
    N_("préparation de la forme d'onde…"),
    N_('relu : %(pct)s %%'),
    N_('suivants +500 ms ⇥'),
    N_('version v%(v)s enregistrée — documents à jour au téléchargement'),
    # --- Choix post-sauvegarde : DOCX rapide vs synthèse resynchronisée ---
    N_("Vos corrections sont dans le verbatim. La synthèse (résumé, décisions) n'est pas encore à jour :"),
    N_('DOCX rapide (synthèse inchangée)'),
    N_('Mettre à jour la synthèse (LLM, quelques minutes)'),
    N_('Mise à jour en cours…'),
    N_('Mise à jour de la synthèse impossible'),
    N_('Mise à niveau demandée…'),
    N_('Mise à niveau terminée — rechargement…'),
    N_('Échec de la mise à niveau : %(e)s'),
    N_('Le service redémarre — reconnexion…'),
    N_("Synthèse mise à jour (versionnée — restaurable depuis le chat d'affinage)."),
    N_('Télécharger le DOCX à jour'),
    N_('Voir le détail dans le chat'),
    N_("Votre navigateur ne permet pas l'enregistrement audio."),
    N_('À relever (ex. montants budgétaires explicitement cités)'),
    N_('À valider'),
    N_('À vérifier %(done)s/%(total)s'),
    N_('Écouter ce segment'),
    N_('⇤ suivants −500 ms'),
    N_('✂ — couper au curseur · ⧉ — fusionner avec le précédent'),
    # job_result_page.js — panneau d'affinage (msgid inchangés, ex-bloc inline A3)
    N_('Proposition'),
    N_('Appliquer cette proposition'),
    N_('Version'),
    N_('Affinage appliqué (version v{v}) — les documents ci-dessus sont régénérés à chaque téléchargement et incluent vos modifications.'),
    N_("Décrivez la modification à appliquer, ou cliquez « Appliquer cette proposition » sous une réponse de l'assistant."),
    N_("Écrivez d'abord votre demande."),
    N_('Appliquer aux documents :'),
    N_('Une version restaurable sera créée (retour arrière possible).'),
    N_('Erreur'),
    N_('Erreur réseau — réessayez.'),
    N_('Options invalides'),
    N_('Restaurer la version v{v} ? Les documents reviendront à cet état.'),
    N_('Restauration impossible'),
    # schedule_page.js — page Planification (msgid inchangés, ex-bloc inline A3)
    N_("Impossible de modifier l'agenda."),
    N_('Modification impossible.'),
    N_('Sélectionne au moins un jour.'),
    N_('Supprimer ce créneau ?'),
    # queue_page.js — page File d'attente (msgid inchangés, ex-bloc inline A3)
    N_('Action impossible sur ce job.'),
    N_('Annuler ce job ?'),
    N_('La priorité doit être comprise entre 1 et 100.'),
    N_('Supprimer tous les jobs de test ?'),
    N_('Les jobs en cours seront ignorés.'),
    N_('Nettoyage impossible.'),
    N_('job(s) de test supprimé(s).'),
    # dashboard_status_page.js — page /system (msgid inchangés, ex-bloc inline A3)
    N_('tout local'),
    N_('Aucune ressource distante configurée (tout intégré).'),
    N_('injoignable'),
    N_('Nœud de ressources injoignable — les transcriptions distantes sont mises en file. Requis : {reqs}.'),
    N_('distant'),
    N_('GPU :'),
    N_('GPU {i} — libre {free} / {total} Mo'),
    N_('Aucun moteur déclaré.'),
    N_('erreur'),
    N_('Statut indisponible ({e}).'),
    # admin_models_page.js — page Modèles (msgid inchangés, ex-bloc inline A3)
    N_('Go'),
    N_('téléchargement…'),
)


def build_js_catalog(locale: str) -> str:
    """Rend le corps JS ``window.I18N = {…}; window.I18N_LOCALE = "xx";``.

    ``locale`` sert au marqueur exposé au front (et au débogage) ; la traduction elle-même
    utilise la locale déjà résolue pour la requête par Flask-Babel.
    """
    catalog = {source: gettext(source) for source in JS_MESSAGES}
    return (
        "window.I18N = " + json.dumps(catalog, ensure_ascii=False) + ";\n"
        "window.I18N_LOCALE = " + json.dumps(locale) + ";\n"
    )
