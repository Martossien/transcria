...
│   │   ├── moss_transcriber.py    # MossTranscriber — MOSS-TD 0,9B une passe ASR+locuteurs (worker TF5 isolé)
│   │   ├── _moss_worker.py        # Worker subprocess MOSS (PYTHONPATH → moss_site Transformers 5)
│   │   └── remote_transcriber.py  # Backend distant/servi (AsrClient) — repli natif via fallback_backend
...
**MOSS-Transcribe-Diarize 0,9B** (`moss_transcriber.py` + `_moss_worker.py`, backend expérimental `moss`, Apache-2.0) transcrit avec **étiquettes locuteur et timestamps fins en une passe** (`[t][Sxx]texte[t]`) — meilleur WER texte du banc de réunions réelles, seule LLM audio unifiée qui tient les fenêtres de 5 min sans boucler. Il exige Transformers 5.x : même patron que `cohere_tf5` — worker subprocess dont le PYTHONPATH pointe d'abord vers un site isolé (`moss.moss_site`, créé par `pip install --target <dir> "transformers>=5,<6" git+https://github.com/OpenMOSS/MOSS-Transcribe-Diarize.git` — torch vient du venv). Deux modes worker : fichier entier (étiquettes une-passe posées en `speaker` + `moss_speaker`) et tours pyannote batchés (`transcribe_prechunked`, modèle chargé UNE fois, locuteur du tour conservé). **Garde des trous inter-segments** (`gap_alert_s`, défaut 10 s) : son vice mesuré est l'omission silencieuse (saut de ~22 s observé, timestamps restant monotones) — le segment aval est marqué `transcription_gap_before_s` et la liste remonte en métadonnées ; on signale, on ne supprime ni n'invente jamais. Pas de forçage de langue (prompt d'entraînement fixe) — la langue du job n'est qu'une métadonnée.

**Runtimes STT servis — audio.cpp (`qwen3asr`) et parakeet.cpp (`nemotron`)** : deux runtimes C++ gérés comme llama-server. Pas de transcriber dédié — le routage passe par `inference.stt.backends.<nom>.url` + `RemoteTranscriber` ; le **démarrage à la demande** est fait par le pré-vol des jobs : en all-in-one (URL loopback + moteur homonyme déclaré dans `resource_node.engines`), `resource_gate._ensure_local_served_stt` appelle `build_stt_supervisor().ensure_ready()` EN PROCESS (cycle A/B/C, admission VRAM par le planner) ; en split, `/engines/ensure` du nœud (inchangé). Santé par moteur : `health_path`/`health_mode` du manifeste (`http_any` réservé aux serveurs mono-modèle chargeant leurs poids avant de binder le port). Builds ÉPINGLÉS par l'installeur (`installer.cli audiocpp|parakeetcpp` → `runtimes/<nom>/{src,bin,COMMIT}`, arch CUDA `native`) ; modèles via le catalogue (`_SERVED_STT_SOURCES` : kind `runtime` délégué au model_manager d'audio.cpp, kind `gguf` classique pour Nemotron). Repli natif par `fallback_backend` — sans lui, erreur explicite. Qualifiés : qwen3asr 0,421 WER, nemotron 0,492 (cf. `docs/EXTERNAL_STT_RUNTIMES.md` et le benchmark).
...
| Méthode | Description |
|---|---|
| `create_transcriber(config, backend, device) -> BaseTranscriber` | Instancie le transcriber selon le backend demandé (`cohere`, `cohere_tf5`, `whisper`, `granite`, `parakeet`, `voxtral`, `kroko` ou `moss`) |
...
| `generate_quick_summary(job, audio_path, gpu_index)` | SileroVAD pré-transcription via `build_speech_chunks()` → transcription des chunks VAD : **en un seul appel batché** (`transcribe_prechunked`, modèle chargé une fois, timestamps absolus) pour les backends à worker (moss, cohere_tf5), sinon boucle par chunk avec `audio_array` → sauvegarde quick_transcript.txt et summary.json → retourne dict (`transcript_text`, `transcript_short`, `summary_text`, `segment_count`) |
...
| `web/editor_routes.py` | Page atelier `/jobs/<id>/editor` + API : `state` (tout en un appel — y compris la **couche fiabilité 0.4.2** : segments non-ok de `transcription_segments.json` et suppressions de `removed_hallucinations.json`), `draft` (verrou optimiste 409, conflit par `base_srt_sha256`), `save` (garde de forme, snapshot POOL COMMUN avec l'affinage : SRT + stats + mapping, purge du brouillon, audit `job_srt_edit_save`), `audio/stream` (Range 206), `peaks` (202→200) |
| `static/js/srt_editor.js` (fiabilité, 0.4.2) | Appariement segment↔fiabilité par recouvrement ≥ 40 % + détection `[INCERTAIN]`, badge et navigation « douteux suivant », repères sur la fresque, panneau des segments supprimés par l'anti-hallucination avec **restauration** (delta miroir de la suppression → undo/redo cohérent) |
...
Le pré-remplissage de l'étape 6 utilise les lexiques globaux et les lexiques des groupes du propriétaire du job. `context/selected_lexicons.json` mémorise les lexiques cochés pour le job ; absent, tous les lexiques accessibles sont sélectionnés. `prefilter_lexicon_entries_for_display()` masque avant affichage les entrées centrales normales sans occurrence dans le transcript/résumé, tout en conservant les priorités `critique`/`importante`. Il ajoute `_display_reason` (`term_presence`, `variant_presence`, `priority`) pour expliquer dans l'UI pourquoi un terme est proposé. Les termes suspects issus du recoupement avec les documents présentés (résumé §6.10) portent `source: "document"` et un badge « issu des documents fournis » (suggestion validée par l'humain, jamais appliquée d'office). Une session déjà sauvegardée reste prioritaire et n'est pas écrasée. Avant correction, `WorkflowRunner.run_correction()` écrit `context/session_lexicon_filtered.json` : termes présents dans le SRT par forme ou variante, plus entrées `critique`/`importante` conservées en préservation.
...
| `run_correction(srt_path, context_path, lexicon_path, invite_path=None)` | Correction SRT : lit transcription.srt + job_context.yaml + lexique filtré, écrit transcription_corrigee.srt + correction_report.md. Si `invite_path` existe (brief + documents présentés), ajoute une clause « référence d'ORTHOGRAPHE des entités nommées uniquement, jamais autorité de contenu » (garde anti-correction-infidèle) |
