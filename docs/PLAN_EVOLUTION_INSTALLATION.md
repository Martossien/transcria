...
`CUDA_VER_FROM_SMI`) et la synthèse VRAM par carte utilisée par le placement LLM
(`GPU_VRAM_TOTAL_MB`, `GPU_VRAM_MAX_MB`, `GPU_SIZES_CSV`) sont isolées dans
`transcria.install_hardware`, avec parsing testé de `nvidia-smi`.
