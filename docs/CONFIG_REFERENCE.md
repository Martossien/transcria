...
| `first_admin_password` | string | `""` | Mot de passe du premier admin. Vide ou sentinelle (`CHANGE-ME`, `admin-change-me`) → le portail demande la création du compte administrateur à la première visite (page `/setup`) ; une valeur = compte créé au premier démarrage sans page (automatisation) |
...
| `csrf_tokens` | bool | `false` | Protection CSRF **forte** : jeton synchroniseur en session exigé à chaque requête mutante par cookie (injecté auto dans formulaires + `fetch` par `static/js/csrf.js`, validé par `web/csrf.py`). L'API par jeton `Bearer` reste exemptée — les scripts doivent utiliser un jeton d'API. |
| `csp` | string | `"off"` | Content-Security-Policy : `off` \| `report-only` (signale sans bloquer) \| `enforce`. Verrouille cadres/objets/base/formulaires/origines. `script-src` garde `'unsafe-inline'` (handlers inline) — cf. `docs/SECURITY_MODEL.md` §3. Déployer en `report-only` d'abord. |
