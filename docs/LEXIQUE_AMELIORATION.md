...
| 6 | Ajouter `contexts` pour afficher 1 à 3 extraits de validation dans l'UI | `summary_prompt.txt`, `opencode_runner.py`, `lexicon.py`, `job_wizard.html`, `wizard.js`, `audio/excerpts.py` | moyen UX | **Implémenté** — `contexts` sauvegardé dans `session_lexicon.json` (3 max, `_normalize_contexts()`), affiché dans l'UI avec bouton lecture/pause, extrait WAV 5s avant/après via `GET /api/jobs/<id>/audio/excerpt`, flag `listened` persisté, cache `metadata/audio_excerpts/*.wav`, réparation des contextes mal parsés et réancrage par citation quand le timecode LLM est approximatif |
