...
| **Voxtral-Mini-4B-Realtime** (audio.cpp, Q8_0 GGUF) | **not scored yet** *(coverage first — see below)* | 5-min window: ~44 s offline / ~76 s streaming — **full 46-min meeting: offline OOMs, streaming ~1× real-time** | one card, ~5 GB weights **+ input-length-dependent** | Mistral's realtime model, added upstream. Coverage depends entirely on how you serve it: **streaming** (`stream=true` SSE, TTFT ~1.4 s) is the only path that scaled to a whole real meeting — it covered our 46-min recording start to finish, continuous, no drops, at ~1× real-time (RTF ~1.0). **Whole-file offline** is memory-bound by input length: clean on a 5-min window (~0.15 RTF) but **CUDA-OOMs on 46 min** (tries to allocate ~44 GB on a 32 GB card — it sizes the compute buffer to the whole input, no internal windowing). **Per-chunk offline** (one VAD segment per request, our pipeline's shape) returns HTTP 200 + **empty text** on a large fraction of ~25 s clips, silently dropping 15–45 % of a meeting — the same span in a longer clip transcribes fine. Also the odd non-Latin (Russian) hallucination on pathological turns, which our automated drift counter scored 0 (only reading caught it). Net: serve it streaming, or window offline at ~5 min — never per-chunk, never whole-file on long audio. WER pending |
...
| **MOSS-Transcribe-Diarize 0.9B** (transformers 5.x, greedy, one 24 GB card) | **0.41** *(8/8, zero empty, zero EN drift)* | 65–104 s | one card, ~2 GB weights | **best text WER of the entire benchmark** — and it emits speaker labels + fine-grained timestamps in the same single pass (`[t][Sxx]text[t]`); speaker counts exact on 2/8 windows (9/9, 5/5), close on most (5/6, 6/7), under-counted on the chaotic ones (3/8, 6/11); Apache-2.0; best single-window score of the whole test (0.19). Reading the outputs (not just scoring them) found its one vice: a **silent 22 s skip** mid-window on W05 — ~50 words of a long monologue omitted, then it resumes verbatim on the next turn; timestamps stay monotonic so nothing *looks* wrong. Easy to detect automatically (gap between consecutive segments), impossible to see in a WER table |
...
- **Serving *granularity* changes the answer, not just the runtime.**
  ...
  test that path explicitly. Length flips it a second time: whole-file offline is
  memory-bound and **CUDA-OOMs on a full 46-min meeting** (it sizes the compute
  buffer to the whole input, ~44 GB on a 32 GB card), while the streaming path
  holds bounded memory and covered the same meeting end-to-end at ~1× real-time.
  So on this realtime model the safe ways to serve it are **streaming**, or
  **offline windowed at ~5 min** — never per-chunk, never whole-file on long
  audio. Three modes, three different answers, same model and runtime.
- **Automatic language detection is the model-side twin of our trap #1.** The
  ...
  real-world audio.
- **Unified audio LLMs keep failing the same exam — except one.** Audex
  30B-A3B is the second joint audio-text giant (after VibeVoice-ASR) that
  transcribes clip-sized excerpts beautifully and then loops without recovery
  on real 5-minute meeting windows; both need their vendor's exact client
  recipe to behave at all (thinking disabled, special tokens blocked,
  temperature escalation…). Then **MOSS-Transcribe-Diarize 0.9B broke the
  pattern**: 8/8 full windows, no loops, no language drift, the best text WER
  of the whole benchmark — and speaker labels plus timestamps in the same
  pass, at a thirtieth of Audex's parameter count on a single card. The
  lesson isn't "unified models can't do meetings"; it's that surviving
  long-form real audio is a training objective you can see instantly in a
  bench like this one — and parameter count has nothing to do with it.
