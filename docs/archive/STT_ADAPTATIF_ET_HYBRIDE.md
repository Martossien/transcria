...
**Première brique du corpus (en place) :** `JobService.analyze()` persiste un résumé audio compact dans `jobs.extra_data_json["audio_summary"]` (scalaires agrégés : `risk_level`, `flags`, `snr_db`, `squim`, `dnsmos`, `difficulty.degrade_ratio`…, **sans** la frise par fenêtre — voir `docs/DATA_MODEL.md`). Cela rend l'**échantillonnage cross-jobs requêtable** (ex. « tous les jobs `degrade_ratio > 0.5` ») sans rejouer les préflights.

**Deuxième brique du corpus (livrée — 2026-06-07) :** l'autre moitié du couple, le **résultat STT par segment**, est désormais persistée. `Transcriber._write_stt_corpus()` (module pur `transcria/stt/corpus.py`) **joint** chaque segment transcrit à la `difficulty_map` par fenêtre (`difficulty_for_range()` : pire niveau + union des signaux chevauchant `[start, end]`) et écrit `metadata/stt_corpus.json` — une ligne par segment : `{start, end, backend, n_words, avg_logprob, no_speech_prob, word_conf_mean, low_word_conf_ratio, reliability, reliability_reasons, difficulty, difficulty_signals, quality_measure}`. Un agrégat compact (`summarize_corpus()` : contingence `difficulté × fiabilité`, moyennes de confiance) est promu dans `extra_data.stt_corpus_summary` pour le requêtage cross-jobs. Activé par défaut (`workflow.stt_corpus.enabled`).

**Troisième axe — qualité mesurée (proxy, livré — 2026-06-07) :** le champ `quality_measure` est désormais rempli par un **proxy automatique de WER** : le taux d'édition mot-à-mot entre le SRT **brut** et le SRT **corrigé** (`segment_edit_rate()` via opcodes `difflib`, normalisé minuscule/ponctuation, borné 1.0). `WorkflowRunner._enrich_stt_corpus_quality()` tourne en **début de phase qualité** (donc après correction *et* relecture finale → SRT corrigé définitif), apparie chaque segment au bloc SRT par **timecode** (la correction préserve les timestamps ; `parse_srt_blocks()` + recherche dichotomique, quasi-linéaire), réécrit `metadata/stt_corpus.json` et recalcule `stt_corpus_summary` (ajoute `quality_measure_mean` et `edit_rate_mean` par niveau de difficulté). Best-effort, **invisible** (pas de step pipeline dédié), ~49 ms mesurés sur un job 1h52 (1582 segments). **Limite assumée :** la correction est volontairement conservatrice → le proxy sous-estime le WER absolu ; il reste exploitable en **relatif** (comparer le taux d'édition entre niveaux de difficulté / moteurs). Le jeu de données cible `difficulté_segment × moteur × qualité_mesurée` a donc ses **trois axes capturés** ; une vérité terrain annotée pourra remplacer le proxy plus tard, même schéma.

**Premier signal mesuré (2 jobs ACRONYME réels, audio Zoom bande étroite) :** difficulté `degrade` → `edit_rate_mean` ≈ 0.0025 vs `ok` ≈ 0.0008, **98 % des segments inchangés** par la correction. Lecture : la difficulté acoustique sur-classe nettement (l'audio « difficile » donne une transcription quasi intégralement acceptée) — c'est exactement le type de constat que ce corpus doit rendre chiffré pour relâcher les seuils.
