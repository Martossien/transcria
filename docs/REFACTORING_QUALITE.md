...
#### ✅ C2 — Découpe d'`opencode_runner.py` *(effort M — LIVRÉE 2026-07-15, 2 lots)*

> **Réalisation** : lot 1 = `gpu/prompt_locator.py` (politique de langue + résolution des
> prompts : `resolve_output_language`, `resolve_prompt_file`, `language_directive`,
> `_SUMMARY_MARKERS`/`summary_markers`, `build_harmonization_glossary`) — corps déplacés à
> l'identique, ré-exportés par opencode_runner (les ~15 consommateurs et les tests importent
> chez lui, zéro site modifié). Lot 2 = `gpu/llm_parsing.py` (les 18 statiques pures :
> parsing résumé/participants/termes suspects/données structurées + sonde métriques vLLM) ;
> la classe garde des délégateurs `staticmethod` (les ~66 sites de tests passent par elle).
> opencode_runner.py : **1543 → 812 l.** (DoD < 900) — il ne reste que sous-processus,
> timeouts, pré-garde endpoint et watchdog, ensemble à dessein. llm_parsing 95 %,
> prompt_locator 91 % ; pas de `LlmTaskExecutor` spéculatif, comme prévu (§9).
