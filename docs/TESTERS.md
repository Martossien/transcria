...
| | |
|---|---|
| **Disk space** | Docker `:latest` (slim): **~22 GB** image + **~12 GB** of model weights downloaded on first start. Docker `:bundled`: **~60 GB** image, weights baked in incl. Voxtral and both LLM tiers (works offline). Native install: plan **100 GB SSD** (the jobs directory grows with your audio). |
...
| **GPU floor** | Compute capability **≥ 7.5** and **≥ 8 GB VRAM** — under 12 GB the smaller 4B LLM tier is picked automatically (the quickstart checks this and fails early with a clear message). |
