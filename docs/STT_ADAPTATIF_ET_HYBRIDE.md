...
**Premier signal mesuré (2 jobs DRITE réels, audio Zoom bande étroite) :** difficulté `degrade` → `edit_rate_mean` ≈ 0.0025 vs `ok` ≈ 0.0008, **98 % des segments inchangés** par la correction. Lecture : la difficulté acoustique sur-classe nettement (l'audio « difficile » donne une transcription quasi intégralement acceptée) — c'est exactement le type de constat que ce corpus doit rendre chiffré pour relâcher les seuils.
