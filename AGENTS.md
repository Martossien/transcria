...
transcria/
  ...
  Dockerfile.bot            # Bot NAVIGATEUR de réunion (Chromium headless) — JAMAIS dans l'image par défaut (poids de Chromium)
  Dockerfile.zoom-sdk       # Bot ZOOM natif (Meeting SDK Linux, 207 Mo de .so) — seule voie ÉPROUVÉE en réunion réelle
  docker/                   # zoom_sdk_entrypoint.sh (amorçage D-Bus/audio : sans lui le SDK SEGFAULTE, sans message)
                            #   + zoom_sdk_verify_libs.py (garde-fou de BUILD : nomme la lib manquante au lieu de la découvrir en réunion)
                            # ⚠ Ne pas construire ces images à la main : `scripts/bot.sh` choisit l'image, la construit si absente,
                            #   règle le mode réseau et lit ~/.transcria-bot.env. Cf. docs/BOT_REUNION.md, docs/DOCKER.md
  docker-compose.bot.yml    # Bot navigateur, un conteneur ÉPHÉMÈRE par réunion (`run --rm bot <url>`) — pas l'image Zoom
  docker-compose.legacy-gpu.yml # Surcharge : `runtime: nvidia` au lieu de CDI, pour un démon Docker < 25 (docker.io Debian/Ubuntu, LXC imbriqué)
                            # RÈGLE DE RELEASE (C7) : tout Dockerfile MODIFIÉ est buildé avant le tag (au minimum `docker build --target <étage modifié>`) — la CI ne parse ni bundled ni resource-node. Gardes texte : tests/test_docker_sync.py (SHAs épinglés == constantes Python, blocs stt-runtimes-builder identiques, .dockerignore, ET tout Dockerfile/compose CITÉ dans une doc — un artefact introuvable n'existe pas pour qui reprend le projet) ; release bundled : scripts/release_bundled.sh UNIQUEMENT (build + vérif contenu conteneur + push GHCR sur --push)
  ...
  transcria/
    ...
    context/
      ...
      meeting_type_models.py / meeting_type_store.py / meeting_type_catalog.py / meeting_type_routes.py / meeting_type_prompts.py # types de réunion PERSONNALISÉS (modèle SQL, store RBAC/quotas, catalogue fusionné 18 types+customs, API /api/meeting-types, variables de prompts) — cf. docs/archive/TYPES_REUNION_PERSONNALISES.md
    quality/
...
Plan directeur : `docs/archive/REFACTORING_QUALITE.md` (🏁 les 15 vagues sont livrées — les
...
- **Résumé persisté** : `JobService.analyze()` écrit un résumé audio **compact** dans `jobs.extra_data_json["audio_summary"]` (`risk_level`, `flags`, `duration_s`, `snr_db`, `squim`, `dnsmos`, `difficulty` agrégé — **sans** la frise par fenêtre, qui reste dans `metadata/audio_preflight.json`). But : requêter/échantillonner à travers les jobs pour calibrer l'arbitrage STT (première brique d'un corpus difficulté ↔ moteur ↔ qualité, cf. `docs/archive/STT_ADAPTATIF_ET_HYBRIDE.md`).
...
**Anti-hallucination STT :** tous les backends locaux (`CohereTranscriber`, `WhisperTranscriber`, `GraniteTranscriber`, `VoxtralTranscriber`, `KrokoTranscriber`, `MossTranscriber`, `CohereTf5Transcriber`) appliquent `collapse_repetition_loops` depuis `anti_hallucination.py` avec leurs sections de config respectives. Les backends partagent la même logique de détection/réduction des boucles répétitives.
...
**Voxtral expérimental :** `models.stt_backend=voxtral` active Mistral Voxtral Mini 3B (Apache-2.0, non-gated, transformers ≥ 4.57 + `mistral-common[audio]`). Langue forcée nativement via `apply_transcription_request` (piège : un ndarray exige `format=["wav"]` et `return_tensors="pt"`). Chemin local `models/voxtral-mini-3b-2507/` sinon repli identifiant HF (cache). Meilleur WER des backends pipeline sur le banc réel ; secondaire par défaut du multi-STT ciblé. Métadonnées dans `metadata/voxtral.json`.

**Kroko expérimental (CPU pur) :** `models.stt_backend=kroko` active Kroko-ASR (Banafo, community CC-BY-SA) sur sherpa-onnx — SEUL backend sans GPU (`vram_mb=0` ⇒ aucune réservation : garde `required_mb <= 0` dans `_reserve_gpu_phase`/`_gpu_session`, `gpu_index=None` propagé). Un modèle zipformer2 PAR langue (~155 Mo), conteneur `.data` maison extrait au premier chargement dans `kroko.model_dir` ; résolution dossier local → cache HF → téléchargement à la demande, repli de variante 128 → 64. Segments construits depuis les timestamps par token. Métadonnées dans `metadata/kroko.json`.

**MOSS expérimental (une passe ASR+locuteurs) :** `models.stt_backend=moss` active MOSS-Transcribe-Diarize 0,9B (Apache-2.0). Exige Transformers 5.x ⇒ worker subprocess (`_moss_worker.py`) sur site isolé `moss.moss_site` (patron `cohere_tf5` : `pip install --target`, torch du venv). Voie batchée `transcribe_prechunked` = tous les tours pyannote en UNE invocation worker (jamais un subprocess par tour : ~15 s de chargement modèle). Son vice mesuré est l'omission silencieuse : garde `moss.gap_alert_s` (trou inter-segments) ⇒ `transcription_gap_before_s` sur le segment aval + liste en métadonnées — on signale, on ne corrige JAMAIS automatiquement. Étiquettes une-passe en `moss_speaker` (+ `speaker` en fichier entier) ; la diarisation pipeline reste pyannote/Sortformer. Métadonnées dans `metadata/moss.json`.

**Runtimes STT SERVIS (audio.cpp `qwen3asr` / parakeet.cpp `nemotron`) :** cycle de vie géré comme llama-server — cf. `docs/EXTERNAL_STT_RUNTIMES.md`. Backends ROUTÉS (pas de transcriber dédié) : `inference.stt.backends.<nom>.url` + `RemoteTranscriber` ; le repli natif passe par `fallback_backend` (backend servi sans repli ⇒ erreur explicite, JAMAIS Cohere implicite). **Démarrage automatique** : en all-in-one, URL loopback + moteur homonyme dans `resource_node.engines` ⇒ le pré-vol des jobs (`resource_gate._ensure_local_served_stt`, couture `client is None`) lance le moteur EN PROCESS (cycle A/B/C du superviseur) ; en split, `/engines/ensure` inchangé. Santé PAR MOTEUR : `health_path`/`health_mode` du manifeste (parakeet-server n'a pas de /v1/models → `/health` ; `http_any` réservé aux serveurs mono-modèle qui chargent leurs poids avant de binder — JAMAIS pour un vLLM). Binaires sous `runtimes/<nom>/` (gitignoré, `TRANSCRIA_RUNTIMES_DIR`), épinglage = constantes `*_PINNED_COMMIT` des phases (monter de version ⇒ nouveau SHA + `--force` + RE-QUALIFICATION sur le benchmark). Ports 8021/8022 (couverts par `stop_stt.sh`), `gpu_mem` du manifeste = ADMISSION seulement (ces serveurs ignorent STT_GPU_MEM), placement GPU = admin (défaut 0 percute la LLM du service). `get_backend_vram_mb` rend 0 pour tout backend routé remote.
...
Cf. `docs/archive/EDITEUR_SRT_INTEGRE.md` (cadrage + lots + retours utilisateur tracés). Règles :
...
Cf. `docs/archive/TYPES_REUNION_PERSONNALISES.md` (cadrage + suivi des lots). Les 18 types du
...
**Ne pas écrire ce fichier à la main** : utiliser `venv/bin/python scripts/setup_opencode.py` (idempotent, format correct, ne casse pas une config existante). Helpers sous-jacents : `transcria/llm_tools/opencode_setup.py` (`find_opencode_binary`, `ensure_local_provider`) et `transcria/gpu/arbitrage_endpoint.py` (`resolve_arbitrage_endpoint`) — découpe du rangement 2026-07-30. Format **courant** produit (≠ ancien `providers`/`type:openai`/`validate`) :
```json
{
...
- **Charge GPU** : tout changement de concurrence GPU/verrous rejoue la campagne (`scripts/bench/load_test.py --jobs 3` en all-in-one, cf. `docs/archive/PLAN_TEST_CHARGE.md`) — elle a attrapé 5 vrais bugs de concurrence à ce jour.
...
| `docs/archive/UI_REUNIONS_WORKFLOW.md` | Plan directeur du parcours RÉUNIONS (vagues 0-5, toutes livrées) : planification, runner, étape 5 |
| `docs/archive/VAGUE5_PISTES_SEPAREES.md` | Pistes séparées, STT par piste, sous-diarisation des micros partagés, suivi en direct provisoire |
